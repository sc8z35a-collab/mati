...
window.EvercityLighting = class EvercityLighting {
  ...
  static installSunFilter(T) {
    ...
    if (!chunk.includes(marker))
      throw new Error("Unsupported Three.js shadow shader");
    const filter = `
  ...
  constructor({
    ...
    this.up = new T.Vector3();
    this.focus = new T.Vector3();
    this.worldUp = new T.Vector3(0, 1, 0);
    const makeSpot = (color, angle, range, shadow = false) => {
      const light = new T.SpotLight(color, 0, range, angle, 0.85, 2);
    ...
    this.streetLights = Array.from({ length: 6 }, () =>
      makeSpot("#ffdba5", 1.15, 27),
    );
    this.roomLights = Array.from({ length: 6 }, (_, i) =>
      makeSpot("#ffe6bf", 1.2, 17, i < 2),
    );
    this.headlights = Array.from({ length: 2 }, () =>
      makeSpot("#d9edff", 0.46, 32),
    );
    sun.shadow.autoUpdate = false;
    this.bounce = new T.PointLight("#ffe4c5", 0, 35, 2);
    scene.add(this.bounce);
    this.fill = new T.DirectionalLight("#a7d4e2", 0.12);
    this.fill.position.set(90, 100, -160);
    scene.add(this.fill);
    const canvas = document.createElement("canvas");
    canvas.width = canvas.height = 128;
    const ctx = canvas.getContext("2d"),
      g = ctx.createRadialGradient(64, 64, 0, 64, 64, 64);
    g.addColorStop(0, "rgba(255,234,186,.75)");
    g.addColorStop(0.12, "rgba(255,226,162,.23)");
    g.addColorStop(1, "rgba(255,216,142,0)");
    ctx.fillStyle = g;
    ctx.fillRect(0, 0, 128, 128);
    const tex = new T.CanvasTexture(canvas);
    this.halos = this.streetLights.map(() => {
  ...
  setQuality(value) {
    this.quality = ["hdr-ultra", "ultra", "high", "balanced"].includes(value)
      ? value
      : "high";
    this.span = { "hdr-ultra": 112, ultra: 112, high: 140, balanced: 170 }[
      this.quality
    ];
    const localSize = Math.min(
      this.renderer.capabilities.maxTextureSize,
      this.quality === "balanced" ? 512 : 1024,
    );
    for (const light of [
      ...this.streetLights,
      ...this.roomLights,
      ...
      if (light.shadow.mapSize.x !== localSize) {
        light.shadow.map?.dispose();
        light.shadow.map = null;
        light.shadow.mapSize.set(localSize, localSize);
      }
  ...
  followSun(mode, weather) {
    ...
    if (sun.userData.solarDirection)
      direction.copy(sun.userData.solarDirection).normalize();
    else
      direction
        .set(mode === "day" ? 90 : -130, mode === "day" ? 300 : 200, 95)
        .normalize();
    right.crossVectors(this.worldUp, direction).normalize();
    ...
    const x = Math.round(focus.dot(right) / texel) * texel,
      y = Math.round(focus.dot(up) / texel) * texel,
      z = Math.round(focus.dot(direction) / texel) * texel;
    sun.target.position
      .copy(right)
      .multiplyScalar(x)
    ...
    const angular =
      weather === "clear" ? 0.0047 : weather === "cloudy" ? 0.014 : 0.022;
    sun.shadow.radius =
      this.quality === "balanced"
        ? 1
        : (this.quality === "hdr-ultra" ? 200 : 100) +
          (angular * 899) / (2 * this.span);
  }
  update(dt, building, mode, weather = "clear") {
    ...
    const p = this.player,
      night = mode === "night",
      golden = mode === "golden",
      indoor = !!building && p.floor < building.floors;
    const interval = {
      "hdr-ultra": 1 / 30,
      ultra: 1 / 30,
      high: 1 / 20,
    ...
    if (this.shadowElapsed >= interval) {
      ...
      for (const actor of [...this.vehicles, ...this.people]) {
        const near =
          (actor.g.position.x - p.x) ** 2 + (actor.g.position.z - p.z) ** 2 <
          range * range;
        actor.g.traverse((mesh) => {
          if (mesh.isMesh) mesh.castShadow = near && !mesh.material.transparent;
        });
      }
      ...
      for (const l of [
        ...
        if (l.castShadow && l.intensity > 0) l.shadow.needsUpdate = true;
      this.renderer.shadowMap.needsUpdate = true;
    ...
    this.elapsed = 0;
    const nearby = [...this.streetFixtures].sort(
      (a, b) =>
        (a.x - p.x) ** 2 +
        (a.z - p.z) ** 2 -
        ((b.x - p.x) ** 2 + (b.z - p.z) ** 2),
    );
    const retained = this.streetLights
      .map((l) => l.userData.fixture)
      .filter((f) => f && Math.hypot(f.x - p.x, f.z - p.z) < 27);
    const selected = [...new Set([...retained, ...nearby])].slice(0, 6);
    this.streetLights.forEach((light, i) => {
      ...
      const changed = light.userData.fixture !== f;
      light.userData.fixture = f;
      if (changed) light.intensity = 0;
      light.position.set(f.x, 6.35, f.z);
      light.target.position.set(f.x, 0.12, f.z + 0.4);
      const distance = Math.hypot(f.x - p.x, f.z - p.z),
        target = !indoor
          ? (night ? 420 : golden ? 65 : 0) *
            this.T.MathUtils.smoothstep(32 - distance, 0, 12)
          : 0;
      light.intensity += (target - light.intensity) * 0.2;
      light.castShadow =
        i <
          (this.quality === "hdr-ultra"
            ? 6
        ...
        night &&
        !indoor;
      this.halos[i].position.copy(light.position);
      this.halos[i].material.opacity = (light.intensity / 420) * 0.34;
    });
    const fixtures = indoor
      ? [...(building.lights?.[p.floor] || [])]
          .filter(
            (f) =>
              f.enabled !== false &&
              (building.type !== "residential" ||
                Math.abs(p.x - building.x) < 3.4 ||
                Math.abs(f.x - building.x) < 3.4 ||
                Math.sign(f.x - building.x) === Math.sign(p.x - building.x)),
          )
          .sort(
            (a, b) =>
              (a.x - p.x) ** 2 +
    ...
    this.roomLights.forEach((light, i) => {
      ...
      if (f) {
        ...
        light.target.position.set(f.x, f.y - 4.5, f.z);
        light.color.set(f.color);
      }
    });
    this.bounce.intensity = indoor ? (night ? 16 : 9) : 0;
    if (indoor)
    ...
    const nearestCars = [...this.vehicles].sort(
      (a, b) =>
        (a.g.position.x - p.x) ** 2 +
        (a.g.position.z - p.z) ** 2 -
        ((b.g.position.x - p.x) ** 2 + (b.g.position.z - p.z) ** 2),
    );
    this.headlights.forEach((light, i) => {
      const v = nearestCars[i];
      if (!v) return;
      const x = v.g.position.x,
        z = v.g.position.z,
        dx = -Math.sin(v.g.rotation.y),
        dz = -Math.cos(v.g.rotation.y);
      light.position.set(x + dx * 2, 0.83, z + dz * 2);
      light.target.position.set(x + dx * 20, 0.15, z + dz * 20);
      light.intensity = night && !indoor ? 190 : 0;
      light.castShadow = this.quality === "hdr-ultra" && night && !indoor;
    });
    for (const light of [
      ...this.streetLights,
      ...this.roomLights,
  ...
  snapshot() {
    return {
      ...
      pcssSamples:
        this.quality === "hdr-ultra"
          ? [32, 64]
          : [12, this.quality === "ultra" ? 24 : 16],
      localShadowSize: this.roomLights[0].shadow.mapSize.x,
      filter: this.quality === "balanced" ? "PCF" : "PCSS",
      shadowSize: this.sun.shadow.mapSize.x,
      shadowTexelMeters: (2 * this.span) / this.sun.shadow.mapSize.x,
      shadowUpdates: this.shadowUpdates,
      registeredStreetFixtures: this.streetFixtures.length,
      streetLights: this.streetLights.filter((l) => l.intensity > 0).length,
      streetShadows: this.streetLights.filter((l) => l.castShadow).length,
      roomLights: this.roomLights.filter((l) => l.intensity > 0).length,
      roomShadows: this.roomLights.filter((l) => l.castShadow).length,
      headlights: this.headlights.filter((l) => l.intensity > 0).length,
      dynamicCasters: [...this.vehicles, ...this.people].filter((a) =>
        a.g.children.some((m) => m.castShadow),
      ).length,
    };
  }
};
...
window.EvercityHDR = class EvercityHDR {
  constructor(T, renderer, camera) {
    Object.assign(this, { T, renderer, camera });
    this.quality = "balanced";
    ...
    this.screenCamera = new T.OrthographicCamera(-1, 1, 1, -1, 0, 1);
    ...
    this.composite = new T.ShaderMaterial({
      ...
      void main(){
        ...
        float ao=1.-clamp(occlusion/samples*aoStrength*3.*(1.-smoothstep(70.,95.,-p.z)),0.,.42);
  ...
  setQuality(value) {
    this.quality = value;
    this.enabled = this.supported && value !== "balanced";
  ...
  resize() {
    ...
    const samples = this.captureActive
      ? 0
      : Math.min(
          this.quality === "hdr-ultra" ? 4 : 2,
    ...
    this.target.samples = samples;
    this.bloomA = new T.WebGLRenderTarget(
      Math.max(1, w >> 2),
      Math.max(1, h >> 2),
    ...
    if (!complete) {
      this.release();
      this.enabled = false;
      console.warn("HDR framebuffer unavailable; using direct ACES rendering.");
      return;
    }
  ...
  finish(mode, source = this.target.texture) {
    ...
    b.extract.value = 1;
    b.stepUV.value.set(1 / this.bloomA.width, 0);
    r.setRenderTarget(this.bloomA);
    r.render(this.screen, this.screenCamera);
    b.source.value = this.bloomA.texture;
    b.extract.value = 0;
    b.stepUV.value.set(0, 1 / this.bloomA.height);
    r.setRenderTarget(this.bloomB);
    r.render(this.screen, this.screenCamera);
    this.quad.material = this.composite;
    const u = this.composite.uniforms;
    u.sceneColor.value = source;
    u.sceneDepth.value = this.target.depthTexture;
    u.bloom.value = this.bloomB.texture;
    u.inverseProjection.value.copy(this.camera.projectionMatrixInverse);
    u.projectionScale.value = this.camera.projectionMatrix.elements[5] * 0.5;
    u.samples.value =
      this.quality === "hdr-ultra" ? 64 : this.quality === "ultra" ? 16 : 8;
    u.aoStrength.value =
      this.quality === "hdr-ultra"
        ? 0.85
        : this.quality === "ultra"
          ? 0.7
          : 0.5;
    u.bloomStrength.value = mode === "night" ? 0.1 : 0.055;
  ...
  async capture(
    ...
    if (!this.enabled)
      throw Error("HDRを利用できません。軽量品質を選んでください。");
    const { T, renderer: r } = this,
      w = width,
      h = height,
      gl = r.getContext();
    ...
    if (
      ...
      maxViewport[1] < renderH
    )
      throw Error(
        "このGPUではHDR撮影用の描画領域を確保できません。通常撮影をご利用ください。",
      );
    const original = {
      camera: this.camera,
      size: r.getSize(new T.Vector2()),
      ratio: r.getPixelRatio(),
      exposure: r.toneMappingExposure,
      autoClear: r.autoClear,
      clearColor: r.getClearColor(new T.Color()),
      clearAlpha: r.getClearAlpha(),
    };
    const photoCamera = EvercityPhotography.camera(this.camera, w / h);
    const sampleCount = this.quality === "hdr-ultra" ? 8 : 2;
    const tileCount = Math.ceil(w / tileW) * Math.ceil(h / tileH);
    let sum, accumulate;
    const output = document.createElement("canvas");
    output.width = w;
    output.height = h;
    const context = output.getContext("2d");
    if (!context) throw Error("4K画像の保存領域を確保できませんでした。");
    const check = () => {
      if (signal?.aborted)
        throw new DOMException("撮影を中止しました。", "AbortError");
      if (gl.isContextLost())
        throw Error(
          "GPUの描画が中断されました。ページを再読み込みしてください。",
        );
    };
    const yieldFrame = () =>
    ...
    try {
      ...
      this.release();
      this.camera = photoCamera;
      r.setPixelRatio(1);
      r.setSize(renderW, renderH, false);
      this.resize();
      check();
      if (!this.enabled || !this.target)
        throw Error("4K HDR用メモリを確保できませんでした。");
      if (
        gl.drawingBufferWidth !== renderW ||
        gl.drawingBufferHeight !== renderH
      )
        throw Error("4K描画領域を確保できませんでした。");
      sum = new T.WebGLRenderTarget(renderW, renderH, {
        type: T.HalfFloatType,
        depthBuffer: false,
      });
      r.setRenderTarget(sum);
      if (gl.checkFramebufferStatus(gl.FRAMEBUFFER) !== gl.FRAMEBUFFER_COMPLETE)
        throw Error("HDR合成用メモリを確保できませんでした。");
      accumulate = new T.ShaderMaterial({
        depthTest: false,
        depthWrite: false,
        toneMapped: false,
        transparent: true,
        ...
        uniforms: { current: { value: null } },
        fragmentShader: `
          varying vec2 vUv;uniform sampler2D current;
          void main(){gl_FragColor=vec4(texture2D(current,vUv).rgb/${sampleCount}.,0.);}`,
      });
      ...
      for (let y = 0; y < h; y += tileH)
        for (let x = 0; x < w; x += tileW) {
          ...
          r.setClearColor(0, 0);
          r.clear();
          r.setClearColor(original.clearColor, original.clearAlpha);
          for (let i = 0; i < sampleCount; i++) {
            progress(
              `写真 / タイル ${tile + 1}/${tileCount} · サンプル ${i + 1}/${sampleCount}`,
              ((tile * sampleCount + i) / (tileCount * sampleCount)) * 0.9,
            );
            await yieldFrame();
            check();
            photoCamera.setViewOffset(
              w,
              h,
              x - pad + offsets[i][0],
              y - pad + offsets[i][1],
            ...
            r.setRenderTarget(this.target);
            r.render(scene, photoCamera);
            this.quad.material = accumulate;
            accumulate.uniforms.current.value = this.target.texture;
            r.autoClear = false;
            r.setRenderTarget(sum);
            r.render(this.screen, this.screenCamera);
            r.autoClear = original.autoClear;
          }
          ...
          r.setRenderTarget(this.target);
          r.render(scene, photoCamera);
          this.finish(mode, sum.texture);
          check();
          // Copy synchronously before the non-preserved WebGL drawing buffer is cleared.
          const cw = Math.min(tileW, w - x),
            ch = Math.min(tileH, h - y);
          context.drawImage(r.domElement, pad, pad, cw, ch, x, y, cw, ch);
          tile++;
        }
      const encoded = new Promise((resolve, reject) =>
        output.toBlob(
          (blob) =>
            blob ? resolve(blob) : reject(Error("PNGの生成に失敗しました。")),
          "image/png",
        ),
      );
      progress("4K PNGを書き出し中", 0.95);
      ...
      return {
        ...
        samples: sampleCount,
        tiles: tileCount,
        shadowSize: scene.children.find(
          (o) => o.isDirectionalLight && o.castShadow,
        )?.shadow.mapSize.x,
        colorSpace: "sRGB / SDR (ACES from linear HDR)",
      };
    } finally {
      r.setRenderTarget(null);
      sum?.dispose();
      ...
      r.setClearColor(original.clearColor, original.clearAlpha);
      this.quad.material = this.composite;
      this.camera = original.camera;
      r.toneMappingExposure = original.exposure;
      // Drop the large attachments before restoring the interactive resolution.
      this.release();
      r.setPixelRatio(original.ratio);
      r.setSize(original.size.x, original.size.y, false);
  ...
  snapshot() {
    return {
      ...
      quality: this.quality,
      msaa: this.target?.samples || 0,
      aoSamples:
        this.quality === "hdr-ultra" ? 64 : this.quality === "ultra" ? 16 : 8,
      pipeline: this.enabled
        ? "RGBA16F / ACES / SSAO / BLOOM"
        : "DIRECT / ACES",
      resolution: this.target ? [this.target.width, this.target.height] : null,
      exposure: this.renderer.toneMappingExposure,
      sceneCalls: this.lastSceneCalls,
    };
  }
};
